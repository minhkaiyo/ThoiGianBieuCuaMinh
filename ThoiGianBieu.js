...
async function saveDataToFirebase() {
    ...
    if (user) {
        try {
            const userDocRef = doc(db, 'userData', user.uid); // Tạo tham chiếu đến document của user
            await setDoc(userDocRef, appData); // Ghi đè toàn bộ document bằng appData hiện tại
            console.log('Dữ liệu đã được lưu lên Firebase!');
        } catch (error) {
            console.error("Lỗi khi lưu dữ liệu:", error);
        }
    } else {
        console.log("Chưa đăng nhập, không thể lưu dữ liệu.");
    }
}

/**
 * Tải dữ liệu của người dùng từ Firestore.
 * Nếu chưa có, tạo dữ liệu mặc định cho họ.
 */
async function loadUserData() {
    ...
    if (user) {
        ...
        if (docSnap.exists()) {
            // Nếu người dùng đã có dữ liệu, tải về
            console.log("Đã tải dữ liệu từ Firebase!");
            appData = docSnap.data(); // Cập nhật biến appData toàn cục
        } else {
            // Nếu là người dùng mới, tạo dữ liệu mặc định cho họ
            console.log("Người dùng mới, tạo dữ liệu mặc định và lưu lên Firebase.");
            // Lúc này, appData đang là dữ liệu mặc định có sẵn trong code
            await saveDataToFirebase();
        }

        // Sau khi có dữ liệu, vẽ lại toàn bộ giao diện
        renderAll();
    }
}
...
//     root.render(React.createElement(AuthComponent));
// }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function attachEventListeners() {
        ...
        // --- LOGIC CHO KHU VỰC THỜI KHÓA BIỂU ---
        ...
        modal.addEventListener('click', function (e) {
            ...
            if (e.target.id === 'save-btn') {
                // **PHẦN MỚI 2: Thêm 'case' để xử lý lưu ngày tháng**
    ...
    function saveDetailedDayChanges(dayKey) {
        ...
        document.querySelectorAll('#detailed-activities-list [data-activity-index]').forEach(row => {
            ...
            if (activity) { // Chỉ lưu nếu có tên hoạt động
    ...
    function renderDetailedScheduleContent() {
        ...
        detailedContainer.innerHTML = ''; // Luôn xóa nội dung cũ trước khi vẽ lại
        ...
        // Thêm class grid vào container nếu nó chưa có
        if (!detailedContainer.classList.contains('grid')) {
            detailedContainer.className += ' grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4';
        }
    ...
    function setDynamicBackground() {
        ...
        // Xóa các lớp nền cũ trước khi thêm lớp mới
        body.classList.remove('day-bg', 'sunset-bg', 'night-bg');

        if (currentHour >= 5 && currentHour < 17) { // 5:00 AM - 4:59 PM
            body.classList.add('day-bg');
        } else if (currentHour >= 17 && currentHour < 19) { // 5:00 PM - 6:59 PM
            body.classList.add('sunset-bg');
        } else { // 7:00 PM - 4:59 AM
    ...
    function attachDynamicEventListeners() {
        ...
        timeAllocCard?.addEventListener('click', (e) => {
            ...
            if (themeOptionBtn) {
                ...
                renderTimeAllocationChart(); // Chỉ cần vẽ lại biểu đồ là đủ
            }
        });
