...
// THAY THẾ TOÀN BỘ ĐỐI TƯỢNG chartThemes CŨ BẰNG ĐỐI TƯỢNG NÀY
const chartThemes = {
    vibrant: {
        name: 'Rực Rỡ',
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderHeaderAndStats() {
        ...
        if (diffDays > 0) {
            countdownElement.innerHTML = `⏱️ ${diffDays} ngày học tập`;
            footerSummaryElement.innerHTML = `🎯 Mục tiêu: A+ tất cả các môn | 📅 ${diffDays} ngày còn lại | 🔥 Let's crush it!`;
        } else {
            countdownElement.innerHTML = `🎉 Hoàn thành mục tiêu!`;
            footerSummaryElement.innerHTML = `🎉 Chúc mừng bạn đã hoàn thành mục tiêu!`;
        }
        const totalSubjects = Object.keys(appData.subjects).length;
    ...
    function renderScheduleHead() {
        ...
        let headHTML = '<tr class="border-b-2 border-gray-200"><th class="p-3 text-left font-bold text-gray-700">⏰ Giờ</th>';
        for (const [key, value] of Object.entries(days)) {
            headHTML += `<th class="p-3 text-center font-bold text-gray-700">${value} <button class="edit-day-btn text-xs" data-day-key="${key}">✏️</button></th>`;
        }
        headHTML += '</tr>';
    ...
    function renderSchedule() {
        ...
        const timeSlots = appData.schedule.timeConfig;
        ...
        Object.keys(timeSlots).forEach(slotKey => {
            ...
            row.innerHTML = `<td class="p-3 font-semibold text-gray-700">${slot.name}<br><span class="text-xs text-gray-500">${slot.time}</span></td>`;
    ...
    function renderOtherSections() {
        ...
        container.innerHTML = `
            <div class="grid md:grid-cols-2 gap-6 mb-8">
                <div id="subjects-section" class="glass rounded-2xl p-6">
                ...
                <div id="time-allocation-card" class="glass rounded-2xl p-6">
                    <h3 class="heading-font text-xl font-bold mb-4 text-gray-800">
    ...
    function renderTimeAllocationChart() {
        ...
        // --- Logic render các nút chọn theme ---
        ...
        // --- Plugin tùy chỉnh để vẽ các hiệu ứng đặc biệt ---
        const customEffectsPlugin = {
            ...
            afterDraw: (chart) => {
                ...
                if (activeElements.length > 0) {
                    ...
                    // Lấy theme hiện tại để vẽ con trỏ
            ...
            beforeDatasetsDraw: (chart) => {
                ...
                // Hiệu ứng glow cho theme Công nghệ
                if (theme.glowColor) {
    ...
    function saveStrategiesChanges() {
        ...
        document.querySelectorAll('#modal-body [data-strategy-index]').forEach(el => {
            if (el.style.display === 'none') return; // Bỏ qua nếu đã bị xóa

            const title = el.querySelector('.strategy-title').value.trim();
            if (title) { // Chỉ lưu nếu có tiêu đề
                newStrategies.push({
    ...
    function attachEventListeners() {
        ...
        // --- LOGIC CHO THỜI KHÓA BIỂU ---
        // Sử dụng event delegation cho toàn bộ khu vực TKB
        const scheduleSection = document.querySelector('.glass:has(#schedule-section)');
        scheduleSection?.addEventListener('click', (e) => {
            // Nút chỉnh sửa TKB cũ (dạng bảng)
            const editDayBtn = e.target.closest('.edit-day-btn');
            if (editDayBtn) {
                currentEditingContext = 'schedule';
                // ================= SỬA LỖI TẠI ĐÂY =================
                // Đã sửa từ e.currentTarget.dataset.key thành editDayBtn.dataset.dayKey
                openEditModal(editDayBtn.dataset.dayKey);
                // ===================================================
                return;
            }

            // Nút thu/mở TKB mới (dạng thẻ)
            ...
            if (toggleDetailedBtn) {
                ...
                toggleDetailedBtn.innerHTML = detailedContainer.classList.contains('expanded') ? '&times;' : '+';
                return;
            }

            // Nút chỉnh sửa TKB mới (dạng thẻ)
        ...
        document.body.addEventListener('click', () => document.getElementById('theme-options')?.classList.remove('visible'));
        ...
        modal.addEventListener('click', function (e) {
            ...
            if (e.target.id === 'add-activity-btn') {
                ...
                newRow.className = 'p-3 border rounded-lg grid grid-cols-1 md:grid-cols-3 gap-2';
                newRow.dataset.activityIndex = newIndex;
                newRow.innerHTML = `
                <input type="text" class="modal-input md:col-span-1" placeholder="VD: 8:00-9:00">
                <input type="text" class="modal-input md:col-span-2" placeholder="Hoạt động">
                <select class="modal-select md:col-span-2">${categoryOptions}</select>
                <button class="delete-activity-btn bg-red-100 text-red-600 rounded px-2 py-1 text-xs">Xóa</button>
            `;
        ...
        document.querySelectorAll('.glass input[type="checkbox"]').forEach(checkbox => {
        ...
        const columnIndex = (dayIndex === 0) ? 8 : dayIndex + 1;
        const scheduleTable = document.querySelector('table');
        if (scheduleTable) {
            scheduleTable.querySelector(`thead th:nth-child(${columnIndex})`)?.classList.add('current-day-header');
            scheduleTable.querySelectorAll('tbody tr').forEach(row => {
                row.querySelector(`td:nth-child(${columnIndex})`)?.classList.add('current-day-highlight');
            });
        }
    }
